...
for link in armChain.links:
    if 'joint_'+str(number) in link.name:
        motor = supervisor.getDevice(link.name)
        motor.setVelocity(1.0)
        position_sensor = supervisor.getDevice('joint_'+str(number)+'_sensor')
...
for positionSensor in position_sensors:
    print('position sensor: ', positionSensor.getValue())
    

# Get the arm and target nodes.
target = supervisor.getFromDef('TARGET1')
...
while supervisor.step(timeStep) != -1:
    ...
    # Call "ikpy" to compute the inverse kinematics of the arm.
    initial_position = [0] + [ps.getValue() for ps in position_sensors] + [0, 0]
    ikResults = armChain.inverse_kinematics([x, y, z], max_iter=IKPY_MAX_ITERATIONS, initial_position=initial_position)
